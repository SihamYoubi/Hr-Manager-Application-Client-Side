...
import { seeLeaveRequests } from '../services/LeaveRequestService';
import { visuallyHidden } from '@mui/utils';

function createData(id, type, startDate, endDate, requestDate, status) {
  return { id, type, startDate, endDate, requestDate, status };
}
    ...
    seeLeaveRequests()
      .then((data) => {
        const formatted = data.map((item) =>
          createData(
            item.id,
            item.typeOfLeave || 'N/A',
            formatDate(item.startDate),
            formatDate(item.endDate),
            formatDate(item.requestDate),
            item.statusLeave || 'N/A'
          )
        );
        setRows(formatted);
      })
